...
function startBossFight() {
  ...
  /* Joystick de movimiento: un solo círculo grande que se arrastra en
     cualquier dirección; el ángulo decide izquierda / derecha / agachar.
     Reemplaza los tres botones pequeños del dpad, más cómodo con el pulgar. */
  const tcJoystick = document.getElementById('tc-joystick');
  if (tcJoystick) {
    const jStick = tcJoystick.querySelector('.tc-joystick-stick');
    let jBase = null;
    const J_DEADZONE = 14;
    const J_MAX_R = 32;

    function setMoveDirection(left, right, down) {
      input.left = left;
      input.right = right;
      input.down = down;
    }
    ...
    function updateJoystickFromDelta(dx, dy) {
      ...
      const clamped = Math.min(len || 0, J_MAX_R);
      const ang = Math.atan2(dy, dx);
      if (jStick) {
        jStick.style.transform = len > 0
          ? `translate(${Math.cos(ang) * clamped}px, ${Math.sin(ang) * clamped}px)`
          : 'translate(0,0)';
      }
      if (len < J_DEADZONE) { setMoveDirection(false, false, false); return; }
      ...
      setMoveDirection(isLeft, isRight, isDown);
    }
    ...
    const jMove = (e) => {
      ...
      e.preventDefault();
      const t = e.touches ? e.touches[0] : e;
      updateJoystickFromDelta(t.clientX - jBase.x, t.clientY - jBase.y);
    };
    const jEnd = (e) => {
      if (e) e.preventDefault();
      jBase = null;
      tcJoystick.classList.remove('pressed');
      setMoveDirection(false, false, false);
      if (jStick) jStick.style.transform = 'translate(0,0)';
    };

    tcJoystick.addEventListener('pointerdown', jStart);
    tcJoystick.addEventListener('pointermove', jMove);
    tcJoystick.addEventListener('pointerup', jEnd);
    tcJoystick.addEventListener('pointerleave', jEnd);
    tcJoystick.addEventListener('pointercancel', jEnd);
  }
  ...
  /* Alinea los controles táctiles con el área real del canvas (puede
     quedar centrado con franjas negras al forzar la relación de aspecto
     en pantalla completa horizontal), no con todo el contenedor. */
