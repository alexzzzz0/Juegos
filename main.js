...
const game = new GameClass(canvas, {
  ...
  onHud(players, level, mode) {
    ...
    hud.innerHTML = `${cards}<div class="stage-card"><strong>${level.name}</strong><em class="progress-info">RECORRIDO ${progress}% · CONTROL ${level.activeCheckpoint}/2</em><em class="timer${timeClass}">TIEMPO ${timeLeft.toString().padStart(3, '0')}</em></div>`;
  },
...
/* Coloca los controles táctiles usando TODO el contenedor (no solo el
   canvas), y calcula cuánto espacio negro queda a los lados cuando la
   relación de aspecto fija del juego no llena el ancho de un teléfono en
   horizontal. Si sobra una franja usable, el joystick, los botones y la
   pausa se centran ahí (fuera del área jugable). Si la franja es muy
   angosta (teléfonos casi 16:9), caen justo pegados al borde visible del
   juego, como antes, para no quedar flotando en la nada. */
const canvasWrap = document.querySelector('.canvas-wrap');
const tcActions = document.querySelector('.tc-actions');
const MIN_USABLE_BAR = 68; // por debajo de esto no alcanza para un control completo

function layoutTouchControls() {
  if (!touchControls || !canvas || !canvasWrap) return;

  const wrapRect = canvasWrap.getBoundingClientRect();
  const canRect = canvas.getBoundingClientRect();

  touchControls.style.left = '0px';
  touchControls.style.top = '0px';
  touchControls.style.width = wrapRect.width + 'px';
  touchControls.style.height = wrapRect.height + 'px';

  const barLeft = Math.max(0, canRect.left - wrapRect.left);
  const barRight = Math.max(0, wrapRect.right - canRect.right);
  const bottomY = wrapRect.height - 74;

  const joyX = barLeft >= MIN_USABLE_BAR ? barLeft / 2 : Math.min(barLeft + 58, wrapRect.width * 0.5);
  const actX = barRight >= MIN_USABLE_BAR
    ? wrapRect.width - barRight / 2
    : Math.max(wrapRect.width - barRight - 58, wrapRect.width * 0.5);
  const pauseX = barRight >= MIN_USABLE_BAR ? wrapRect.width - barRight / 2 : wrapRect.width - barRight - 24;

  if (tcJoystick) { tcJoystick.style.left = joyX + 'px'; tcJoystick.style.top = bottomY + 'px'; }
  if (tcActions) { tcActions.style.left = actX + 'px'; tcActions.style.top = bottomY + 'px'; }
  if (tcPause) { tcPause.style.left = pauseX + 'px'; tcPause.style.top = '22px'; }
}
window.addEventListener('resize', layoutTouchControls);
window.addEventListener('orientationchange', () => setTimeout(layoutTouchControls, 50));
if (window.ResizeObserver) new ResizeObserver(layoutTouchControls).observe(canvas);
layoutTouchControls();
})();
