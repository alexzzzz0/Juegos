...
const arcadeBase = window.ARCADE_BASE || '';

document.querySelectorAll('[data-arcade-script]').forEach((script) => {
  if (!script.src.includes('/assets/')) return;
  const fileName = script.dataset.arcadeScript;
  const expected = `${arcadeBase}assets/${fileName}`;
  if (!script.src.endsWith(expected)) script.src = expected;
});

function rutaDelJuego(ruta) {
  return `${arcadeBase}${ruta}`;
}
...
function crearTarjeta(juego) {
  ...
  a.className = "cabinet";
  a.href = rutaDelJuego(juego.carpeta);
  ...
  if (juego.preview) {
    if (tipoDePreview(juego.preview) === "video") {
      previewHTML = `<video class="vista-previa" src="${rutaDelJuego(juego.preview)}" muted loop playsinline preload="auto" aria-hidden="true"></video>`;
    } else {
      previewHTML = `<img class="vista-previa" src="${rutaDelJuego(juego.preview)}" alt="" aria-hidden="true" loading="eager">`;
    }
  }
  ...
  if (juego.preview) {
    const activarPreview = () => {
      ...
      if (imagePreview && !imagePreview.src) imagePreview.src = rutaDelJuego(juego.preview);
    };
  ...
  a.addEventListener("click", (e) => {
    ...
    setTimeout(() => {
      window.location.href = rutaDelJuego(juego.carpeta);
    }, 320); 
  });
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", inicializarArcade, { once: true });
} else {
  inicializarArcade();
}
